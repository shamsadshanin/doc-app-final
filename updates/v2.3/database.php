...
if (isset($_POST["btn_admin"])) {
    ...
    if (mysqli_connect_errno()) {
        ...
        mysqli_query($connection, "ALTER TABLE `users` ADD `banner` VARCHAR(255) NULL DEFAULT NULL AFTER `signature`;");
        mysqli_query($connection, "ALTER TABLE `users` ADD `template` INT NOT NULL DEFAULT '2' AFTER `custom_js`;");
        mysqli_query($connection, "ALTER TABLE `settings` ADD `sender_mail` VARCHAR(255) NULL DEFAULT NULL AFTER `mail_port`;");

        // import database table
        $query = '';
          $sqlScript = file('sql/email_templates.sql');
        ...
        ('admin', 'Email Template Settings', 'email-template-settings', 'Email Template Settings'),
        ('admin', 'Overview', 'overview', 'Overview'),
        ('admin', 'Upload Banner', 'upload-banner', 'Upload Banner'),
        ('admin', 'Template', 'template', 'Template'),
        ('admin', 'Select Profile Template', 'select-profile-template', 'Select Profile Template'),
        ('admin', 'Do not bold the variable names or brackets.', 'dont-bold-the-variable-names-or-brackets', 'Do not bold the variable names or brackets.'),
        ('admin', 'Dosing Schedule', 'dosing-schedule', 'Dosing Schedule'),
