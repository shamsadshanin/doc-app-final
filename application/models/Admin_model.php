...
class Admin_model extends CI_Model {
    ...
    function get_all($table, $total, $limit, $offset)
    {
        $this->db->select('*');
        $this->db->from($table);
        if ($this->session->userdata('role') == 'user') {
    ...
    function all_get_drugs($total, $limit, $offset)
    {
        $this->db->select('*');
        $this->db->from('drugs');
        ...
        if (!empty($_GET['search'])) {
            ...
            $this->db->or_like('generic_name', $_GET['search']);
            $this->db->or_like('brand_name', $_GET['search']);
    ...
    {
        $this->db->select('a.*, p.name, p.mobile, p.email, p.mr_number, u.name as dr_name, u.currency, u.email as dr_email');
        $this->db->from('appointments a');
        $this->db->join('patientses p', 'p.id = a.patient_id', 'LEFT');
        $this->db->join('users u', 'u.id = a.user_id', 'LEFT');
